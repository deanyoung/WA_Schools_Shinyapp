...
knn_data <- data.frame(runif(30,0,10))
colnames(knn_data) <- c("x")
knn_data %<>% mutate(y=3*x,error=rnorm(30,0,1),yhat=y+error)
knn_data %<>% mutate(euclid_4 = abs(4-x))
knn_data %<>% mutate(three_closest = ifelse(euclid_4 %in% head(sort(knn_data$euclid_4),3), "Yes","No"))

ggplot(aes(x, yhat), data=knn_data) + geom_point(aes(col=three_closest, size=4)) +
...
knn_data %>% filter(three_closest=="Yes") %>% summarise(mean(yhat))
```

## KNN (K=10)

```{r, echo=FALSE}
knn_data %<>% mutate(ten_closest = ifelse(euclid_4 %in% head(sort(knn_data$euclid_4),10), "Yes","No"))

ggplot(aes(x, yhat), data=knn_data) + geom_point(aes(col=ten_closest, size=4)) +
  geom_smooth(method="lm", se=FALSE) +
  scale_color_manual(values=c("black","red")) + 
  scale_x_continuous(breaks=seq(0,10,2)) +
  geom_vline(xintercept=4, color="red", linetype="dashed") +
...
knn_data %>% filter(ten_closest=="Yes") %>% summarise(mean(yhat))
...
- Predict mathLImet with LowIncome, ELL, and Enrollment
